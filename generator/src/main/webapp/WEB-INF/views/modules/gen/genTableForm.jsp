...
<ul class="nav nav-tabs">
    ...
    <li class="active">
        <a href="${base}/admin/gen/genTable/form?id=${genTable.id}&name=${genTable.name}">业务表
            ${not empty genTable.id?'修改':'添加'}查看
        </a>
    </li>
</ul>
<c:choose>
    <c:when test="${empty genTable.name}">
        <form:form id="inputForm" modelAttribute="genTable" action="${base}/admin/gen/genTable/form" method="post"
    ...
    <c:otherwise>
        <form:form id="inputForm" modelAttribute="genTable" action="${base}/admin/gen/genTable/save" method="post"
                   class="form-horizontal">
            <form:hidden path="id"/>
            <%--<sys:message content="${message}"/>--%>
            <fieldset>
                ...
                <div class="control-group">
                    <table id="contentTable" class="table table-striped table-bordered table-condensed">
                        ...
                        <c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
                            <tr${column.valid eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
                                <td nowrap>
                                    <input type="hidden" name="columnList[${vs.index}].id" value="${column.id}"/>
                                    <input type="hidden" name="columnList[${vs.index}].valid"
                                           value="1"/>
                                ...
                                <td>
                                    <select name="columnList[${vs.index}].queryType" class="required input-mini">
                                        <c:forEach items="${config.queryTypeList}" var="dict">
                                            <option value="${dict.value}" ${dict.value==column.queryType?'selected':''}
                                                    title="${dict.description}">${dict.label}</option>
                                        </c:forEach>
                                    </select>
                                </td>
